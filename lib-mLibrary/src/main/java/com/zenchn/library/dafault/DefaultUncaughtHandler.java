...
public final class DefaultUncaughtHandler implements UncaughtExceptionHandler {
    ...
    public void init(@NonNull Context context, @NonNull ICrashCallback crashCallback, ICrashHandler crashHandler) {
    ...
    public DefaultUncaughtHandler setCrashHandler(@NonNull ICrashHandler mCrashHandler) {
    ...
    public void uncaughtException(Thread thread, Throwable ex) {
        ...
        if (mCrashHandler != null && mCrashHandler.getReportMode()) {
            try {
                //导出异常信息到SD卡中
                File logFile = dumpExceptionToSDCard(ex);
                //这里可以通过网络上传异常信息到服务器，便于开发人员分析日志从而解决bug
                mCrashHandler.uploadExceptionToServer(logFile);
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
