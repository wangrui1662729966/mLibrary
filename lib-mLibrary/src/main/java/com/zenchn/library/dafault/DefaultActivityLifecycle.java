 ...
 * 修订记录：
 */
public class DefaultActivityLifecycle extends Observable implements IActivityLifecycle {
    ...
    public void onActivityStarted(Activity activity) {
        if (refActivityCount == 0) {
            for (int i = mCallbackStack.size() - 1; i >= 0; i--) {
                mCallbackStack.get(i).get().onForeground();//TODO 处于前台时候的回调
            }
        }
        refActivityCount++;
    }
    ...
    public void onActivityStopped(Activity activity) {
        ...
        if (refActivityCount == 0) {
            for (int i = mCallbackStack.size() - 1; i >= 0; i--) {
                mCallbackStack.get(i).get().onBackground();//TODO 处于后台时候的回调
            }
        }
    }
